...
@app.route('/')
def home():
    # Show the disease prediction form as the home page
...
def predict():
    try:
        ...
        print(f"Symptoms: {select1}, {select2}, {select3}, {select4}")
        
        # Check if models are loaded
        if not all([model, model1, model2]):
            return render_template('desease.html', 
                                 predicted_disease=["Model not available"], 
                                 precautions=["Please check server configuration"], 
                                 occurrences="System error")
        ...
        # Get disease info for the first predicted disease
        if predicted_disease and predicted_disease[0] in disease_info:
            precautions = disease_info[predicted_disease[0]][0]
            occurrences = disease_info[predicted_disease[0]][1]
        else:
            precautions = ["Consult a healthcare professional"]
            occurrences = "Please consult a doctor for accurate diagnosis"
        ...
        return render_template('desease.html', 
                             predicted_disease=["Error occurred"], 
                             precautions=["Please try again"], 
                             occurrences="System error")
